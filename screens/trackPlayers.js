...
const TrackPlayers = ({ route, navigation }) => {
  ...
  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql(
        "SELECT playerName, SUM(isAttending) as attended, SUM(isExecused) as excused, COUNT(*) as total FROM trackAttendance GROUP BY playerName;",
        [],
        (_, { rows: { _array } }) => {
          setPlayers(_array);
          //   console.log(_array);
            ...
            <Pressable
              onPress={async () => {
                ...
                await new Promise((resolve, reject) => {
                  db.transaction((tx) => {
                    tx.executeSql(
                      "SELECT week, isAttending as attended, isExecused as excused FROM trackAttendance INNER JOIN track ON trackId = id WHERE playerName = ?;",
                      [item.playerName],
                      (_, { rows: { _array } }) => {
